...
const EditUser = ({ showModal, setShowModal, user }: PropTypes) => {
  ...
  const formSubmit: SubmitHandler<UserForm> = async data => {
    ...
    return showToast({
      message: result.status === "success" ? `User updated successfully` : String(result.error),
      type: result.status,
    });
  };
